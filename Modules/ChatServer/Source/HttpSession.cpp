  ...
  handle_request(beast::string_view doc_root, http::request<Body, http::basic_fields<Allocator>> &&req)
{
  ...
  if (req.method() != http::verb::get && req.method() != http::verb::head) return bad_request("Unknown HTTP-method");
...
HttpSession::HttpSession(tcp::socket &&socket, const std::shared_ptr<Broadcaster> &broadcaster, ChatServer &chatServer)
...
{
  chatServer_->Leave(this);
}

void HttpSession::Run()
{
  chatServer_->Join(this);
  doRead();
}
...
void HttpSession::doRead()
{
  ...
  // Read a request
  http::async_read(
    stream_, buffer_, parser_->get(), beast::bind_front_handler(&HttpSession::onRead, shared_from_this()));
}
...
void HttpSession::onRead(beast::error_code ec, std::size_t)
{
  ...
  if (websocket::is_upgrade(parser_->get())) {
    ...
    std::make_shared<WebSocketSession>(stream_.release_socket(), broadcaster_)->Run(parser_->release());
    return;
  }

  // Handle request
  http::message_generator msg = handle_request("http", parser_->release());
