...
namespace TwitchBot
{
  ...
  static auto parseEmotesTag(std::u8string_view emotes) -> std::optional<EmotesInfo>
  {
    ...
    while (not emotes.empty())
    {
      ...
      while (not positions.empty())
      {
        ...
        int startInt;
        {
          auto [ptr, ec] = std::from_chars(reinterpret_cast<const char *>(start.data()), reinterpret_cast<const char *>(start.data() + start.size()), startInt);
          if (ec != std::errc{})
            return std::nullopt;
        }
        int endInt;
        {
          auto [ptr, ec] = std::from_chars(reinterpret_cast<const char *>(end.data()), reinterpret_cast<const char *>(end.data() + end.size()), endInt);
          if (ec != std::errc{})
            return std::nullopt;
        }
        ...
        if (posIdx == std::u8string_view::npos)
          break;

        positions = positions.substr(posIdx + 1);
      }

      if (idIdx == std::u8string_view::npos)
        break;
  ...
  static auto parseTags(std::u8string_view tags) -> Tags
  {
    ...
    no_parse:
      if (keyIdx == std::u8string_view::npos)
        break;
    }
    return resultTags;
  }

  static auto privateMessageSplitParts(std::u8string_view message, const EmotesInfo &emotesInfo) -> std::vector<MessagePart>
  {
    ...
    for (const auto &position : emotesInfo.Positions)
    {
      ...
      if (not text.empty())
        parts.emplace_back(std::in_place_type<TextPart>, text);
    ...
    if (not message.empty())
      parts.emplace_back(std::in_place_type<TextPart>, message);
  ...
  auto Parse(std::u8string_view ircMessage) -> std::optional<TwitchIRCMessage>
  {
    ...
    message.Parameters = parameters.empty() ? std::optional<std::u8string_view>(std::nullopt) : std::optional<std::u8string_view>(parameters);
    return message;
  }

  auto ParseCommand<IRCCommand::PING>::operator()(std::optional<std::u8string_view> parameters) -> std::optional<CommandParameters<IRCCommand::PING>>
  {
    if (not parameters)
      return std::nullopt;
  ...
  auto ParseCommand<IRCCommand::PRIVMSG>::operator()(
      const TwitchIRCMessage &message) -> std::optional<CommandParameters<IRCCommand::PRIVMSG>>
  {
    auto &[command, tags, source, parameters] = message;
    std::u8string_view displayName;
    const EmotesInfo *emotesInfo = nullptr;
    if (not parameters)
      return std::nullopt;
    ...
    if (tags)
    {
      ...
      if (tagDisplayName)
        displayName = tagDisplayName.value();
      if (emotes)
        emotesInfo = &(emotes.value());
    }
    else
    {
      if (source and source.value().Nick)
        displayName = source.value().Nick.value();
    }
    if (not emotesInfo)
    {
      return CommandParameters<IRCCommand::PRIVMSG>{.DisplayName = displayName};
    }
    else
    {
      return CommandParameters<IRCCommand::PRIVMSG>{.DisplayName = displayName,
                                                    .Parts = privateMessageSplitParts(parameters.value(), *emotesInfo)};
    }
  }
}